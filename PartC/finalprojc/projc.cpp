...
int routeNkeyorder_setup(int routnum10, uint16_t circuitnum, map<uint16_t,struct keyNroutelist *> &circuit_info){ //create route order, generates 6 keys, and assignes them in their order.
    //int number1 = routnum10;
    //initiallize next_router by emptying it
    struct keyNroutelist *list = new struct keyNroutelist;
    
    for(deque<int>::iterator it = list->next_router.begin(); it < list->next_router.end(); it++){
        list->next_router.pop_front();
    }
    /*
     ...
     */
    for(deque<unsigned char *>::iterator its = list->keyorder.begin(); its < list->keyorder.end(); its++){
        list->keyorder.pop_front();
    }
    ...
    unsigned char *rk1 = new unsigned char[16];
    memset(rk1, 0,16);
    for(int i=0; i< 16; i++)
    {
        rk1[i]=aes_key[i] ^ ((unsigned char)0x01);
    }
    unsigned char *rk2 = new unsigned char[16];
    memset(rk2, 0,16);
    for(int i=0; i< 16; i++)
    {
        rk2[i]=aes_key[i] ^ ((unsigned char)0x02);
    }
    unsigned char *rk3 = new unsigned char[16];
    memset(rk3, 0,16);
    for(int i=0; i< 16; i++)
    {
        rk3[i]=aes_key[i] ^ ((unsigned char)0x03);
    }
    unsigned char *rk4 = new unsigned char[16];
    memset(rk4, 0,16);
    for(int i=0; i< 16; i++)
    {
        rk4[i]=aes_key[i] ^ ((unsigned char)0x04);
    }
    unsigned char *rk5 = new unsigned char[16];
    memset(rk5, 0,16);
    for(int i=0; i< 16; i++)
    {
        rk5[i]=aes_key[i] ^ ((unsigned char)0x05);
    }
    unsigned char *rk6 = new unsigned char[16];
    memset(rk6, 0,16);
    for(int i=0; i< 16; i++)
    {
    ...
    //deque<unsigned char *> keyorder;
    list->keyorder.push_back(rk1);
    list->keyorder.push_back(rk2);
    list->keyorder.push_back(rk3);
    list->keyorder.push_back(rk4);
    list->keyorder.push_back(rk5);
    list->keyorder.push_back(rk6);
    
    if(routnum10 < 6){
        int last = 5;
        for(int bf = 6 - routnum10; bf > 0; bf--){
            delete [] list->keyorder[last];
            list->keyorder.pop_back();
            last = last - 1;
        }
    }
    
    for(int t = 0; t < routnum10; t++){ // assignes numbers 1 to num and creates keys
        list->next_router.push_back(t+1);
    }
    ...
    while(routnum10 > 0){
        ...
        temp = list->next_router[direction];
        store = list->keyorder[direction];
        list->next_router.erase(list->next_router.begin() + direction); // erased this element
        list->keyorder.erase(list->keyorder.begin() + direction);
        list->next_router.push_back(temp); //first hop pushed back
        list->keyorder.push_back(store); //first hop pushed back
     ...
     */
    circuit_info[circuitnum] = list;
    
    return 1;
} //routeNkeyorder_setup

int routeNkeyorder_setupstage9(deque<int> &next_router, deque<unsigned char *> &keyorder, int routnum10, unsigned char rk1[16],unsigned char rk2[16],unsigned char rk3[16],unsigned char rk4[16],unsigned char rk5[16],unsigned char rk6[16],int routerdown[6]){ //create route order, generates 6 keys, and assignes them in their order.
    //int number1 = routnum10;
    //initiallize next_router by emptying it
    for(deque<int>::iterator it = next_router.begin(); it < next_router.end(); it++){
        next_router.pop_front();
    }
    /*
     while(!keyorder.empty()){
     keyorder.pop_front();
     ...
     */
    for(deque<unsigned char *>::iterator its = keyorder.begin(); its < keyorder.end(); its++){
        keyorder.pop_front();
    }
    ...
    while(routnum10 > 0){
        ...
        if( !((temp == routerdown[0]) || (temp == routerdown[1]) || (temp == routerdown[2]) || (temp == routerdown[3]) || (temp == routerdown[4]) || (temp == routerdown[5])) ){
    ...
    return 1;
}//routeNkeyorder_setupstage9

int circuit_setup9(int routernum10, struct sockaddr_in &their_addr1, struct sockaddr_in &their_addr2, struct sockaddr_in &their_addr3,struct sockaddr_in &their_addr4, struct sockaddr_in &their_addr5, struct sockaddr_in &their_addr6, deque<int> &next_router,deque<unsigned char *> &keyorder, int hops,int sockfd,uint16_t flownum){
    ...
    if((direction == 1) && (temphops)){
        int nrouter;
        ofile.open("stage9.proxy.out",std::fstream::app);
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 1, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
        for(int z = 0; z < 16; z++){
            uint16_t shade;
            memset(&shade,0,2);
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 2, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 3, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
        for(int z = 0; z < 16; z++){
            uint16_t shade;
            memset(&shade,0,2);
            shade = (uint16_t)firstkey[z];
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 4, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
        for(int z = 0; z < 16; z++){
            uint16_t shade;
            memset(&shade,0,2);
            shade = (uint16_t)firstkey[z];
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 5, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
        for(int z = 0; z < 16; z++){
            uint16_t shade;
            memset(&shade,0,2);
            shade = (uint16_t)firstkey[z];
         ...
         */
        ofile.open("stage9.proxy.out",std::fstream::app);
        ofile << "new-fake-diffie-hellman, router index: 6, circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
        for(int z = 0; z < 16; z++){
            uint16_t shade;
            memset(&shade,0,2);
            shade = (uint16_t)firstkey[z];
    ...
    while(temphops){ // recieve circuit extend messages and send out another control message to that same 1st hop router and breaks when NEXT-NAME is 0xfff
        ...
        if((strcmp(IP_src,"127.0.0.1") == 0) && (strcmp(IP_dest, "127.0.0.1") == 0) && (IP->ip_p == 253)){
            ...
            if(temphops){
                ...
                ofile << ", circuit outgoing: 0x" << setw(2) << setfill('0') << hex << flownum << ", key: 0x";
...
int circuit_setup8(int routernum10, struct sockaddr_in &their_addr1, struct sockaddr_in &their_addr2, struct sockaddr_in &their_addr3,struct sockaddr_in &their_addr4, struct sockaddr_in &their_addr5, struct sockaddr_in &their_addr6, deque<int> &next_router,deque<unsigned char *> &keyorder, int hops,int sockfd,uint16_t flownum){
    ...
    if((direction == 1) && (temphops)){
        int nrouter;
        ofile.open("stage8.proxy.out",std::fstream::app);
        for(int hop_t = 0; hop_t < hops; hop_t++){
            nrouter = next_router[hop_t];
            ofile << "hop: " << hop_t+1 << ", router: " << nrouter << endl;
        }
...
int stage7function(int num, int hops){
...
childcontinues:
    ...
    stage1done:
        ...
        if(stage1){
            ...
            if((direction == 1) && (temphops)){
                int nrouter;
                ofile.open("stage7.proxy.out",std::fstream::app);
                for(int hop_t = 0; hop_t < hops; hop_t++){
                    nrouter = next_router[hop_t];
                    ofile << "hop: " << hop_t+1 << ", router: " << nrouter << endl;
                }
...
int stage8function(int num, int hops){
    deque<unsigned char *> keyorderback; //for decrpytion at proxy
    //deque<int> next_routerback; //Stage 8
    
    map<uint16_t,struct keyNroutelist *> circuit_info;
    deque<struct flow *> flow_info;

    deque<unsigned char *> keyorder;
    deque<int> next_router; //Stage 8
    uint16_t proxyid = 0x0001;
...
childcontinues:
    if (pid == 0) {
        ...
        struct addrinfo h;
        struct addrinfo *info;
        uint16_t myid; // Stage 8 : recalcuate for each new circuit
        //unsigned char mykey[16]; // Stage 8: recalculate for each new circuit
        map<uint16_t,unsigned char *> keylookup; // Stage 8: maps myid circuit to my AES key (new) circuit 'i' not circuit id.
        uint16_t circuitnumber = 0x0000; // Stage 8: keep track of circuit number
        map<uint16_t,uint16_t> cidnext; // key = my circuit id, data = next port number
        map<uint16_t,uint16_t> cidreturn; // key = my circuit id, data = port I should relay back to
        map<uint16_t,uint16_t> cidreturnid; // key = my circuit id, data = circuit id of who I should relay back to
        //int flag1 =1;
                    ...
                    if((strcmp(IP_src,"127.0.0.1") == 0) && (strcmp(IP_dest, "127.0.0.1") == 0) && (IP->ip_p == 253)){
                        ...
                        if(minitorbuf[0] == 0x65){
                            ...
                            if(it != cidnext.end() ){ // should be good
                                ...
                                myid = 0x0100*routernum + (host & 0x00ff); //stage8
                                ...
                                unsigned char *mykey; //Stage8
                                mykey = keylookup[host & 0x00ff]; //Stage8
                                
                                cout << "this is my key: " << mykey << endl;
                                ...
                                unsigned char *mykey = new unsigned char[16]; //stage 8
                                ...
                                keylookup[host & 0x00ff] = mykey; //Stage 8
                            ...
                            if((it != cidnext.end()) && (cidnext[host] != 0x0000)){ // should be good
                                ...
                                myid = 0x0100*routernum + (host & 0x00ff);
                                ...
                                myid = 0x0100*routernum + (host & 0x00ff); //stage 8
                                ...
                                if(nextport == 0xffff){ //Add for Stage 7
                                    ...
                                    if(protocol == IPPROTO_ICMP){
                                        ...
                                        if(((a == e) && (b == f) && (c == g))){   // open brackets this means that this ICMP message is directed to eth1's network
                                            ...
                                            int flag3 = 0;
                                            for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                                if( (!strcmp((*add)->src,tun_ip_src)) && (!strcmp((*add)->dest,tun_ip_dest)) && ((*add)->protocol == IPPROTO_ICMP) && ((*add)->src_port == 0) && ((*add)->dest_port == 0)){ // there exists this circuit.
                                                    flag3 = flag3 + 1;
                                            ...
                                            if(!flag3){
                                                struct flow *nextflow = new struct flow;
                                                nextflow->circuit_num = host & 0x00ff;
                                                memcpy(nextflow->src,tun_ip_src,NI_MAXHOST);
                                                memcpy(nextflow->dest,tun_ip_dest,NI_MAXHOST);
                                                nextflow->protocol = IPPROTO_ICMP;
                                                nextflow->src_port = 0;
                                                nextflow->dest_port = 0;
                                                flow_info.push_back(nextflow);
                                            }
                                            ...
                                            cout << "message is directed to MIT and is ICMP" << endl;
                                            cout << "address of MIT is: " << tun_ip_dest << endl;
                                        ...
                                        if(((a == e) && (b == f) && (c == g))){   // open brackets this means that this ICMP message is directed to eth1's network
                                            ...
                                            if(tun_ICMP->ip_p == IPPROTO_TCP){
                                                ...
                                                int flag3 = 0;
                                                for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                                    if( (!strcmp((*add)->src,tun_ip_src)) && (!strcmp((*add)->dest,tun_ip_dest)) && ((*add)->protocol == IPPROTO_TCP) && ((*add)->src_port == tcp_hdr->source) && ((*add)->dest_port == tcp_hdr->dest)){ // there exists this circuit.
                                                        flag3 = flag3 + 1;
                                                ...
                                                if(!flag3){
                                                    ...
                                                    memcpy(nextflow->src,tun_ip_src,NI_MAXHOST);
                                                    memcpy(nextflow->dest,tun_ip_dest,NI_MAXHOST);
                                                    nextflow->protocol = IPPROTO_TCP;
                                                    nextflow->src_port = tcp_hdr->source;
                                                    nextflow->dest_port = tcp_hdr->dest;
                                                    flow_info.push_back(nextflow);
                                                }
                ...
                if(recv_bytes){
                    ...
                    if(((a == e) && (b == f) && (c == g))){
                        ...
                        if(tun_ICMP->ip_p == IPPROTO_ICMP){
                            ...
                            //( (!strcmp((*add)->src,"0.0.0.0")) && (!strcmp((*add)->dest,tun_ip_src)) && ((*add)->protocol == IPPROTO_ICMP) && ((*add)->src_port == 0) && ((*add)->dest_port == 0))
                            //tun_utype = 0;
                            int flag3 = 0;
                            for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                if( (!strcmp((*add)->src,"0.0.0.0")) && (!strcmp((*add)->dest,tun_ip_src)) && ((*add)->protocol == IPPROTO_ICMP) && ((*add)->src_port == 0) && ((*add)->dest_port == 0)){ // there exists this circuit.
                                    flag3 = flag3 + 1;
                                    circuitnumber = (*add)->circuit_num;
                                }
                            }
                            
                            if(!flag3){
                                cout << "ERROR: no known circuit to relay ICMP packet" << endl;
                                //return 1;
                            }
                            
                            
                            myid = 0x0100*routernum + circuitnumber; //stage 8
                        ...
                        //( (!strcmp((*add)->src,"0.0.0.0")) && (!strcmp((*add)->dest,tun_ip_src)) && ((*add)->protocol == IPPROTO_TCP) && ((*add)->src_port == tcp_hdr->dest) && ((*add)->dest_port == tcp_hdr->source))
                        if(tun_ICMP->ip_p == IPPROTO_TCP){
                            struct tcphdr *tcp_hdr;
                            int header = tun_ICMP->ip_hl;
                            tcp_hdr = (struct tcphdr *)(recbuf + header*4);
                            ...
                            int flag3 = 0;
                            for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                if( (!strcmp((*add)->src,"0.0.0.0")) && (!strcmp((*add)->dest,tun_ip_src)) && ((*add)->protocol == IPPROTO_TCP) && ((*add)->src_port == tcp_hdr->dest) && ((*add)->dest_port == tcp_hdr->source)){ // there exists this circuit.
                                    flag3 = flag3 + 1;
                                    circuitnumber = (*add)->circuit_num;
                                }
                            }
                            ...
                            if(!flag3){
                                cout << "ERROR: no known circuit to relay TCP packet" << endl;
                                //return 1;
                            }
                        ...
                        unsigned char *mykey; //Stage8
                        mykey = keylookup[circuitnumber]; //Stage8
    ...
    stage1done:
        ...
        if(stage1){
            ...
            //eventually this should be implemented when something is read from the tunnel
            ...
            /*
             ...
             ofile.open("stage8.proxy.out",std::fstream::app);
             ofile << "router number is: " << next_router[i] << endl;
             ofile << "key" << "0" << " is: ";
             for(int z = 0; z < 16; z++){
             uint16_t shade;
             memset(&shade,0,2);
             ...
             unsigned char firstkey1[16];
             memset(firstkey1, 0, 16);
             memcpy(firstkey1,keyorder[1],16);
             ofile << "key" << "1" << " is: ";
             for(int z = 0; z < 16; z++){
             uint16_t shade;
             memset(&shade,0,2);
             shade = (uint16_t)firstkey1[z];
             if((shade < 0x000f)){
             ofile << "0";
             }
             if(shade > 0x00ff){
            ...
            //deque<int> &next_router, deque<unsigned char *> &keyorder
            /*
             ofstream fileout;
             fileout.open("keys.out");
             fileout.close();
             cout << "This is the route " << endl;
             deque<int>::iterator it;
             for(it = next_router.begin() ; it < next_router.end(); it++){
             cout << (*it) << " ";
             }
             ...
             deque<unsigned char *>::iterator its;
             for(its = keyorder.begin() ; its < keyorder.end(); its++){
             unsigned char outkey[16];
             memset(outkey,0,16);
             memcpy(outkey,keyorder[cultivate],16);
             
             fileout.open("keys.out",std::fstream::app);
             for(int z = 0; z < 16; z++){
             uint16_t shade;
             memset(&shade,0,2);
             shade = (uint16_t)outkey[z];
             if((shade < 0x000f)){
             fileout << "0";
             }
             if(shade > 0x00ff){
             ...
             fileout << hex << shade; //rk1[z];
             
             }
             fileout << "\n";
             fileout.close();
             //cout << outkey << endl;
             cultivate = cultivate + 1;
             }
             */
            
            //routeNkeyorder_setup(next_router,keyorder,routnum10,rk1,rk2,rk3,rk4,rk5,rk6);
            
            //circuit_setup(routnum10, their_addr1, their_addr2, their_addr3,their_addr4,their_addr5,their_addr6, next_router, keyorder,hops,sockfd);
            ...
            //Stage 8
            uint16_t circuitnumber = 0x0001; // Stage 8: keep track of circuit number
            uint16_t flownumber = 0x0001;
            
            circuitnumber = circuitnumber - 0x0001;
            /* Connect to the tunnel interface (make sure you create the tunnel interface first) */
            strcpy(tun_name, "tun1");
            tun_fd = tun_alloc(tun_name, IFF_TUN | IFF_NO_PI);
            ...
            while(rv) // rv can be equal to -1 (error), 0 (timerout), or other received data
            {
                ...
                if(FD_ISSET(tun_fd, &readfds)){
                    ...
                    else //if(nread >= 60) //cheat method
                    {
                        ...
                        /*
                         if(abcd){
                         routeNkeyorder_setup(routnum10,circuitnumber,circuit_info);
                         
                         keyorder = circuit_info[circuitnumber]->keyorder;
                         next_router = circuit_info[circuitnumber]->next_router;
                         
                         circuit_setup(routnum10, their_addr1, their_addr2, their_addr3,their_addr4,their_addr5,their_addr6, next_router, keyorder,hops,sockfd,circuitnumber);
                         }
                        ...
                        if(tun_ICMP->ip_p == IPPROTO_ICMP){
                            ...
                            int flag3 = 0;
                            for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                if( (!strcmp((*add)->src,tun_ip_src)) && (!strcmp((*add)->dest,tun_ip_dest)) && ((*add)->protocol == IPPROTO_ICMP) && ((*add)->src_port == 0) && ((*add)->dest_port == 0)){ // there exists this circuit. retrieve keys and route path
                                    
                                    flownumber = (*add)->circuit_num;
                                    flag3 = flag3 + 1;
                                    //reset
                                    for(deque<int>::iterator it = next_router.begin(); it < next_router.end(); it++){
                                        next_router.pop_front();
                                    ...
                                    //reset
                                    keyorder = circuit_info[flownumber]->keyorder;
                                    next_router = circuit_info[flownumber]->next_router;
                                }
                            }
                            ...
                            if(!flag3){
                                circuitnumber = circuitnumber + 0x0001;
                                struct flow *nextflow = new struct flow;
                                nextflow->circuit_num = circuitnumber;
                                memcpy(nextflow->src,tun_ip_src,NI_MAXHOST);
                                memcpy(nextflow->dest,tun_ip_dest,NI_MAXHOST);
                                nextflow->protocol = IPPROTO_ICMP;
                                nextflow->src_port = 0;
                                ...
                                circuit_setup8(routnum10, their_addr1, their_addr2, their_addr3,their_addr4,their_addr5,their_addr6, next_router, keyorder,hops,sockfd,circuitnumber);
                            }
                        ...
                        if(tun_ICMP->ip_p == IPPROTO_TCP){
                            ...
                            int flag3 = 0;
                            for(deque<struct flow *>::iterator add = flow_info.begin(); add < flow_info.end(); add++){
                                if( (!strcmp((*add)->src,tun_ip_src)) && (!strcmp((*add)->dest,tun_ip_dest)) && ((*add)->protocol == IPPROTO_TCP) && ((*add)->src_port == tcp_hdr->source) && ((*add)->dest_port == tcp_hdr->dest)){ // there exists this circuit. retrieve keys and route path
                        ...
                        uint8_t type = 0x61;
                        uint16_t hostc_id = circuitnumber;
                        uint16_t netwc_id = htons(hostc_id); //incoming circuit id 2bytes
                ...
                if(FD_ISSET(sockfd, &readfds)) {
                    ...
                    if((strcmp(IP_src,"127.0.0.1") == 0) && (strcmp(IP_dest, "127.0.0.1") == 0) && (IP->ip_p == 253)){
                        ...
                        if(minitorbuf[0] == 0x64){
                            ...
                            //map<uint16_t,struct keyNroutelist *> circuit_info;
                            //struct keyNroutelist reverse;
                            //memset(&reverse,0,sizeof(reverse));
                            //memcpy(&reverse,circuit_info[host & 0x00ff],sizeof(reverse));
                            
                            for(deque<unsigned char *>::iterator its = keyorderback.begin(); its < keyorderback.end(); its++){
                                keyorderback.pop_front();
                            }
                            
                            keyorderback = circuit_info[host & 0x00ff]->keyorder;
                            ...
                            for(int i = 0;  i < hops; i++)
                            {
                                ...
                                unsigned char nextkey[16];
                                memset(nextkey,0,16);
                                memcpy(nextkey,keyorderback[i],16); //storying encryption key in nextkey
...
int stage9function(int num, int hops, int killpacket)
{
    unsigned char rk1[16];
    unsigned char rk2[16];
    unsigned char rk3[16];
...
childcontinues:
    if (pid == 0) {
        ...
        struct addrinfo hintsr;
        struct addrinfo *resr;
        int sockfdr, sockbindr, statusr, getr;
        uint16_t dynamic_portr; //Stage 9
        struct sockaddr_in *ipv4r; //structure for ipv4
        socklen_t addrlen1r = sizeof(*ipv4r);
        char msg[2048];
        int child = (int) getpid();
        ...
        uint16_t myid; // Stage 8 : recalcuate for each new circuit
        unsigned char mykey[16]; // Stage 8: recalculate for each new circuit
        map<uint16_t,unsigned char [16]> keylookup; // Stage 8: maps myid circuit to my AES key (new)
        uint16_t circuitnumber = 0x0001; // Stage 8: keep track of circuit number
        map<uint16_t,uint16_t> cidnext; // key = my circuit id, data = next port number
        map<uint16_t,uint16_t> cidreturn; // key = my circuit id, data = port I should relay back to
        map<uint16_t,uint16_t> cidreturnid; // key = my circuit id, data = circuit id of who I should relay back to
    ...
    child_again:
        ...
        ofiler.open(filename, std::fstream::app);
        ofiler << "this is ICMP_count: " << ICMP_count << endl;
        ofiler.close();
                    ...
                    if((strcmp(IP_src,"127.0.0.1") == 0) && (strcmp(IP_dest, "127.0.0.1") == 0) && (IP->ip_p == 253)){
                        ...
                        if(minitorbuf[0] == 0x65){
                            ...
                            if((it != cidnext.end()) && (cidnext[host] != 0x0000)){ // should be good
                                ...
                                uint16_t port1;
                                memcpy(&port1,clear_crypt_text,2);
                                memcpy(&next_port,clear_crypt_text,2);
                                /*
                                 ofile.open(filename,std::fstream::app);
                                 ofile << "port1 is" << port1 << endl;
                                 ofile.close();
                            ...
                            if(it != cidnext.end() ){ // should be good
                                ...
                                if(nextport == 0xffff){ //Add for Stage 7
                                    ...
                                    if(protocol == IPPROTO_ICMP){
                                        ...
                                        if(((a == e) && (b == f) && (c == g))){   // open brackets this means that this ICMP message is directed to eth1's network
                                            ICMP_count = ICMP_count - 1; //Stage 9
                        ...
                        else if((minitorbuf[0] & 0x00ff) == 0x0091){
                            
                            cout << endl;
                            cout << "ENTERED 0x91" << endl;
                            
                            ofile.open(filename,std::fstream::app); //puts decrypted version first
                            ofile << "router " << routernum << " killed" << endl;
                            ofile.close();
                            gotkilled = 1;
                            _exit(EXIT_SUCCESS);
                            exit(0);
                ...
                if(recv_bytes){
                    ...
                    if(((a == e) && (b == f) && (c == g))){
                        ...
                        if(tun_ICMP->ip_p == IPPROTO_ICMP){
                            ...
                            myid = 0x0100*routernum + circuitnumber;
        ...
        if((ICMP_count > 0) && (!gotkilled)){
            ofiler.open(filename, std::fstream::app);
            ofiler << "this is ICMP_count: " << ICMP_count << endl;
            ofiler << "router " << dec << dynamic_portr << " worried about " << dec << next_port;
            ofiler << " on circuit 0x" << hex << setw(2) << setfill('0') << circuitnumber << endl;
            ofiler.close();
            
            char worried[2048];
            memset(worried, 0, sizeof(worried));
            ...
            int inc = 0;
            memcpy(worried, &minhdr, sizeof(minhdr));
            inc = sizeof(minhdr);
            memcpy(worried+inc, &type, sizeof(type));
            inc = inc + sizeof(type);
            memcpy(worried+inc, &netwc_id, sizeof(netwc_id));
            inc = inc + sizeof(netwc_id);
            
            memcpy(worried+inc, &dynamic_portr, sizeof(dynamic_portr)); //BE CAREFULL HOST TO NETWORK ORDER?
            inc = inc + sizeof(dynamic_portr);
            memcpy(worried+inc, &next_port, sizeof(next_port)); //BE CAREFULL HOST TO NETWORK ORDER?
            inc = inc + sizeof(next_port);
            ...
            sendto(sockfdr, worried, inc, 0, (struct sockaddr *)&previous, sizeof(previous));
        ...
        map<int,int> port_router; // key = port number, data = router number;
        ...
        while((bytes_rec) && num){   //  this waits for the I'm up message before calling tunnel_reader to receive/send packets from tun1
            ...
            *space = '\0';
            ...
            port_router[router_port] = atoi(routerid); //Stage 9
        ...
        if(stage1){
            ...
            //routeNkeyorder_setupstage9(next_router,keyorder,routnum10,rk1,rk2,rk3,rk4,rk5,rk6);
            ...
            //int abcd = 1;
            int routerdown[6];
            memset(routerdown,0,sizeof(routerdown));
            int routerdown_count = 0;
            int createcircuit = 1;
            ...
            while(rv) // rv can be equal to -1 (error), 0 (timerout), or other received data
            {
                    ...
                    else //if(nread >= 60) //cheat method
                    {
                        ...
                        if(createcircuit){
                            while(createcircuit){
                                packetnumber = 1;
                                circuitnumber = circuitnumber + 0x0001;
                                routeNkeyorder_setupstage9(next_router,keyorder,routnum10,rk1,rk2,rk3,rk4,rk5,rk6,routerdown);
                                
                                circuit_setup9(routnum10, their_addr1, their_addr2, their_addr3,their_addr4,their_addr5,their_addr6, next_router, keyorder,hops,sockfd,circuitnumber);
                                createcircuit = createcircuit-1;
                                //sleep(3);
                            }
                        }
                        //abcd = 0;
                        ...
                        if(packetnumber == killpacket){ // Stage 9 TODO
                            ...
                            int inc = 0;
                            memcpy(killmsg, &minhdr, sizeof(minhdr));
                            inc = sizeof(minhdr);
                            memcpy(killmsg+inc, &type, sizeof(type));
                            inc = inc + sizeof(type);
                            memcpy(killmsg+inc, &netwc_id, sizeof(netwc_id));
                            inc = inc + sizeof(netwc_id);
                            
                            int second;
                            second = next_router[1]; //second hop
                            
                            if(second == 1){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr1, sizeof(their_addr1));
                            }
                            else if(second == 2){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr2, sizeof(their_addr2));
                            }
                            else if(second == 3){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr3, sizeof(their_addr3));
                            }
                            else if(second == 4){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr4, sizeof(their_addr4));
                            }
                            else if(second == 5){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr5, sizeof(their_addr5));
                            }
                            else if(second == 6){
                                sendto(sockfd, killmsg, inc, 0, (struct sockaddr *)&their_addr6, sizeof(their_addr6));
                            }
                ...
                if(FD_ISSET(sockfd, &readfds)) {
                    ...
                    if((strcmp(IP_src,"127.0.0.1") == 0) && (strcmp(IP_dest, "127.0.0.1") == 0) && (IP->ip_p == 253)){
                        ...
                        if(minitorbuf[0] == 0x64){
                        ...
                        else if((minitorbuf[0] & 0x00ff) == 0x92){
                            uint16_t cir;
                            uint16_t mine;
                            uint16_t next;
                            ...
                            memcpy(&cir,minitorbuf+1,sizeof(cir));
                            memcpy(&mine,minitorbuf+3,sizeof(mine));
                            memcpy(&next,minitorbuf+5,sizeof(next));
                            
                            ofile.open("stage9.proxy.out",std::fstream::app);
                            ofile << "router-worried message received, circuit 0x" << setw(2) << setfill('0') << hex << ntohs(cir);
                            ofile << ", SELF-NAME: " << dec << mine << ", NEXT-NAME: " << dec << next << endl;
                            ofile.close();
                            
                            uint16_t nextport;
                            memset(&nextport,0,sizeof(nextport));
                            memcpy(&nextport,minitorbuf+5,2);
                            routerdown[routerdown_count] = port_router[(int)nextport];
                            routerdown_count = routerdown_count + 1;
                            createcircuit = 1;
                            packetnumber = 0;
                        }
...
int main(int argc, char** argv)
{
    ...
    string str12 = "stage 7";
    string str13 = "stage 8";
    string str14 = "stage 9";
    ...
    */
    ...
    while(ifile.good())
    {
        ...
        //{
            ...
            {
                if(sItem[0]=="stage")
                    stage=atoi(sItem[1].c_str());
                if(sItem[0]=="num_routers")
                    num=atoi(sItem[1].c_str());
                if(sItem[0]=="minitor_hops")
                    hops=atoi(sItem[1].c_str());
                if(sItem[0]=="die_after")
                    killpacket=atoi(sItem[1].c_str());

            }
            
        //}
    }
    printf("Stage:%d\t Nodes:%d\t Hops: %d\t Die After: %d\n",stage,num,hops,killpacket);
    //fclose(config_fp);
    ifile.close();
    ...
    if(stage == 1) {
        ...
        stage7function(num, hops);
    }
    else if(stage == 8){
        stage8function(num, hops);
    }
    else if(stage == 9){
        stage9function(num, hops,killpacket);
    }
...
/*
    ...
    if( str1 == str2 ) {
        ...
        cout << "\nWe are in Stage7 " << endl;
        stage7 = 1;
    }
    else if(str1 == str13){
        cout << "\nWe are in Stage8 " << endl;
        stage8 = 1;
    }
    else if(str1 == str14){
        cout << "\nWe are in Stage9 " << endl;
        stage9 = 1;
    }
    ...
    if(stage5 || stage6 || stage7 || stage8 || stage9){
    ...
    if(stage9){
        getline(ifile,str15,' '); // get minitor_hops
        ifile.getline(str16,15); // get number of hops

        killpacket = atoi(str16);
        
        cout << "Die after " << killpacket << endl;
    }
